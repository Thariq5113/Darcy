...
custom_chatbot = {
    ...
    # Commands (20)
    "introduce yourself": "Hello! I am Alpha, your personal humanoid robot assistant. I can see,hear,speak and move like a human  I am here to help you answer your questions and interact with the world around me.",
...
def speak(text):
    try:
        ...
        sd.play(data, fs, device=SPEAKER_INDEX)  # set to correct index
        sd.wait()
    except Exception as e:
        print(f"❌ TTS error: {e}")
...
def get_response(text):
    ...
    # 2. Try key containment (e.g., if 'how are you' in text)
    for key in custom_chatbot:
        if key in text:
            response = custom_chatbot[key]
            return response() if callable(response) else response

    # 3. Try fuzzy match (to handle typos or close queries)
